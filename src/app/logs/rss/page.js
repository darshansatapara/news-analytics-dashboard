...
export default function RSSLogsPage() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/logs/rss?${params.toString()}`);
      ...
      console.error("Error fetching RSS logs:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportLogs = () => {
    const csv = [
      ["Timestamp", "Category", "URL", "Articles", "Status", "Error"].join(","),
      ...logs.map((log) =>
        [
    ...
    a.href = url;
    a.download = `rss-logs-${Date.now()}.csv`;
    a.click();
  };
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 mt-1">
                  Monitor and analyze RSS feed fetch operations
                </p>
              </div>
                ...
                {analytics.topFeeds && analytics.topFeeds.length > 0 && (
                  <div className="bg-gray-900/50 backdrop-blur-xl border border-gray-800/50 rounded-2xl p-6">
                    ...
                    <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                      {analytics.topFeeds.slice(0, 10).map((feed, index) => (
                        <div
                          key={index}
                          className="p-4 bg-gray-800/30 rounded-xl border border-gray-700/50 hover:border-gray-600/50 transition-all"
                        >
                    ...
                    {analytics.commonErrors.slice(0, 5).map((error, index) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className="px-3 py-1 bg-red-500/10 rounded-lg text-xs font-semibold text-red-400">
                            {error.count} occurrences
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-xs text-gray-400">
                          <LinkIcon className="w-3 h-3" />
                          <span>Affected URLs: {error.urls?.length || 0}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-gray-900/50 backdrop-blur-xl border border-gray-800/50 rounded-2xl p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2 bg-gray-800/50 border border-gray-700/50 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50"
                    >
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className="px-6 py-2 bg-purple-500 hover:bg-purple-600 rounded-xl font-medium transition-colors"
                  >
                    Apply Filters
                  </button>
                  <button
                    ...
                    {loading ? (
                      ...
                      logs.map((log, index) => (
                        ...
                        >
                          ...
                          <td className="p-4">
                            <span className="px-3 py-1 bg-purple-500/10 border border-purple-500/30 rounded-lg text-xs font-medium text-purple-400 capitalize">
                              {log.category}
                            </span>
                          </td>
